...
\title{\hug 金型空間内の不良成形品ピッキングのための画像認識を用いたロボットシステムの検討}
...
パーセプトロンは, 図\ref{fig:perceptron}[a]に示すような構造を持ち, その最終的な出力は, 入力ベクトルと重みベクトルの外積和を活性化関数で変換したものになる. 例えば, 入力ベクトルが2つの場合, この入力ベクトルと重みベクトルの外積和は次式で求められる.
\[
...
\begin{eqnarray}
	...
	h(x) &=& w_0 + w_1x_1 + w_2x_2 + \cdot + w_nx_n \nonumber \\
		     ...
		     &=& {\bf w}^T{\bf x}
\end{eqnarray}
となる. 入力が画像など二次元で表される場合, その全体の入力および重みベクトルは
\begin{eqnarray}
	\label{vector}
...
\end{eqnarray}
 ...
 $h({\bf x}) $の値が, 活性化関数(式\ref{perceptron_out})における閾値$\theta$以上であれば, パーセプトロンの出力は1であり, 0より小さければその出力は0である.
...
パーセプトロンの目標は, 入力がクラス1に属しているとき出力が1, クラス0に属しているとき出力が0となるような重み${\bf w} $の集合を見つけることである. その重みは, 式\ref{perceptron_learning_low}で表されるパーセプトロン学習則に従い, 自動的に変更される.
\begin{eqnarray}
	\label{perceptron_learning_low}
	{\bf w}_j^{new} = {\bf w}_j^{old} + \Delta {\bf w}_j 
\end{eqnarray}
ここで${\bf w}$, $\Delta {\bf w}_j $, ${\bf x}$は以下のようにあらわすことができ, 正解クラスラベル$y$, 推定されたクラスラベル$y'$, 学習率$\ eta(0 \leq \eta \leq 1)$を用いて,
\begin{eqnarray}
	\label{perceptron_learning_low}
	\Delta{\bf w}_j = \eta(y - y')x_j^i
\end{eqnarray}
ここで${\bf w}', {\bf w}$はそれぞれ更新後と更新前の重み, ${\bf y}$は入力${\bf x}$の正しいクラス番号, ${\bf y}'$はパーセプトロンの出力である. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
パーセプトロンが開発されて数年後, B. Widrowと彼の大学院生たちにより, ADALINEと呼ばれるパーセプトロンの改良版が発明された. 
ADALINEはパーセプトロンの違いは, 活性化関数とコスト関数の概念を導入したことである. 
