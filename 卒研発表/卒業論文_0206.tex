...
\begin{eqnarray}
	...
	h(x) &=& w_0 + w_1x_1 + w_2x_2 + \cdot + w_nx_n \nonumber \\
		     ...
		     &=& {\bf w}^T{\bf x}
\end{eqnarray}
となり, $h({\bf x}) $の値が, 活性化関数(式\ref{perceptron_out})における閾値$\theta$以上であれば, パーセプトロンの出力は1であり, 0より小さければその出力は0である.
\begin{figure} [htbp]
	\begin{center}
		\begin{tabular}{c}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
非線形問題を扱うために, 1986年D. Rumelhartによって考案されたニューラルネットワークが多層パーセプトロンである. 
多層パーセプトロンは, 入力層, 出力層, そして中間層(隠れ層)で構成されており, 入出力層と隠れ層1層の場合が最も単純な構造である.
また, 各層は内部にいくつかのノードと呼ばれる処理要素を持ち, 層ごとにその機能が異なる.

入力層のノードは, 自信と接続されているすべてのノードに対して, 同じ入力データを伝達する役割を持つ.
隠れ層は, パーセプトロンと同様の構造を持つが, 活性化関数に非線形関数を用いている. 
