...
隠れ層のノードは, パーセプトロンと同様の構造を持ち, 活性化関数にシグモイド関数やReLU関数を用いる.
...
そして, 重み訓練を行うニューラルネットワークにとって, その挑戦目標は, 希望のネットワーク性能を与える位置に, 重み${\bf w}$を導く学習則を開発することである. この学習則の多くは, はっきりとした目標のもとに定式化される. 一般に多く見かけるタイプの目標は, ニューラルネットワークの平均2乗和誤差や, 〇〇のような全体的コストまたは評価関数を最小または最大にすることである（このような学習則は性能学習(performance learning)と呼ばれるカテゴリに属する）. ある時間区間の入力信号の平均(または平均以外の性質)を学習するような, 特定のコスト関数の最適化を目標としない学習則は, 典型的な行動学または数学の用語で表すことができる目標を持つ(これはフィルタ学習(filter learning)カテゴリに属する学習則の目標である). 
ここでは, 実験に関係のある, 性能学習について説明する.

例えばパーセプトロンの場合, \ref{sec:perceptron}に示したパーセプトロン学習則において, $({\bf y} - {\bf y}' = 0)$を達成する重みベクトル${\bf w}$を見つけることが目標である. この式\ref{perceptron_learning_low}は以下のHebbの推定した生物学的学習則(Hebbの学習則)より導くことができる.
\begin{equation}
	\label{Hebb_learning_law}
	{\bf w}^{new} = {\bf w}^{old} + {\bf y}_k{\bf x}_k^T  
\end{equation}
この学習則は, 
