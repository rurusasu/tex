...
\section{パーセプトロン}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
パーセプトロンは, 図\ref{perceptron}に示すように, (n+1)次元ベクトル${\bf x} = (x_0, x_1, x_2,  \cdots, x_n)$からなる入力を持つ. 
ここで$x_0$は恒等的に$1$にセットされ(これはバイアス入力と呼ばれる), $y$からは正しい出力が入力される.
...
\begin{figure}[ht]
	\begin{center}
		\includegraphics[width=50mm,clip]{./figure/perceptron.eps}
		\caption{perceptron}
		\label{perceptron}
	\end{center}
\end{figure}

またパーセプトロンは, 複数ある入力信号それぞれに固有の重み$w_i$をもち, その重みは, 各信号の重要性をコントロールする要素として働く.
すなわち, この重みはシナプスの結合強度を表し, 値が大きければ大きいほど, その重みに対応する信号の重要性が高くなる.

パーセプトロンの利点は, 図に示すように, n次元空間内における点を2つのパターンに分類できることである. 
