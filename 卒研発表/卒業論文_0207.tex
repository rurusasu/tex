...
\title{\huge 金型空間内の不良成形品ピッキングのための画像認識を用いたロボットシステムの検討}
%\titlewidth{}% タイトル幅 (指定するときは単位つきで)
\author{三木　康平}
\eauthor{Kohei Miki}% Copyright 表示で使われる
\studentid{F116056}
\supervisor{永田寅臣　教授}% 1 つ引数をとる (役職まで含めて書く)
%\supervisor{指導教員名 役職 \and 指導教員名 役職}% 複数教員の場合，\and でつなげる
\handin{2020}{03}% 提出月. 2 つ (年, 月) 引数をとる
...
\section{機械学習}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
機械学習とは, 「ルールの記述が難しい問題（例えば1枚の画像から人の顔を見分ける問題）」に対して, 人間がルールを決めるのではなく, コンピュータ自体にルールを発見させようする手法」である. 
機械学習の問題は, 「分類問題」と「回帰問題」に大別することができる. 分類問題とはデータがどのクラスに属するか, という問題である. 例えば, 人の写った画像から, その人が男性か女性かのどちらであるかを分類するような問題が分類問題に相当する. 一方, 回帰問題は, ある入力データから, (連続的な)数値の予測を行う問題である. 例えば, 人の写った画像から, その人の体重を予測するような問題が回帰問題に相当する.

機械学習において, コンピュータがルールを発見するまでの過程を「学習」, 発見されたルールをモデルといい,  このモデルを用いて画像や音声などの判定(認識)を行う. 
...
パーセプトロンは, 図\ref{fig:perceptron}[a]に示すような構造を持ち, n次元空間内における点が2クラスに線形分離可能な場合, 有限の試行回数で確実に分類できる. すなわち, この機械は分類問題の一部を解くことができる. その一例を図\ref{fig:perceptron}[b]に示す. ここで, それぞれの点の座標はクラス分けされる対象の属性または特徴を表す. また線形分離可能とは, その2クラスが, 単純な線形超平面(2次元においては直線, 3次元空間においては平面, そしてn次元空間においては$(n-1)$次元平面)で互いに分離できる場合をいう. 

パーセプトロンの最終的な出力は, 入力ベクトルと重みベクトルの外積和を活性化関数で二値化したものになる. 例えば, 入力ベクトルが2つの場合, この入力ベクトルと重みベクトルの外積和は次式で求められる.
\[
h({\bf x}) = w_1x_1 + w_2x_2 + b
\]
ここで, bはバイアスと呼ばれ, 後に説明する活性化関数において出力信号が1を出力する度合いを調整するパラメータとして機能する. パーセプトロンにおいては一般的にバイアスは$b = 1$を入力とする. またパーセプトロンは, 入力信号に1対1で対応する重み${\bf w} = (w_0, w_1, w_2,  \cdots, w_n)$をもち, その重みは, 各信号の重要性をコントロールする要素として働く.
すなわち, この重みはシナプスの結合強度を表し, 値が大きいほど, その重みに対応する信号の重要性が高くなる.
前述の式を$w_0=b, x_0=1$として一般化すると, 
\begin{eqnarray}
	\label{perceptron_OPS}
	h(x) &=& w_0 + w_1x_1 + w_2x_2 + \cdot + w_nx_n \nonumber \\
		     &=& \sum_{i=0}^n w_ix_i \nonumber \\
		     &=& {\bf w}^T{\bf x}
\end{eqnarray}
となり, $h({\bf x}) $の値が, 活性化関数(ヘヴィサイド関数(式\ref{perceptron_out}))における閾値$\theta$以上であれば, パーセプトロンの出力は1であり, 0より小さければその出力は0である.
\begin{figure} [htbp]
	\begin{center}
		\begin{tabular}{c}
...
\begin{equation}
	...
	y'(h(x)) = \left\{
		\begin{array}{l}
			1 \hspace{4mm} (h(x) \geq \theta )\\
			0 \hspace{4mm} (h(x) < \theta)
		\end{array}
		\right.
\end{equation}
入力が画像など二次元で表される場合, その全体の入力および重みベクトルは以下の式で表される.
\begin{eqnarray}
	\label{input_vector}
	{\bf x} &=& (x_{11}, x_{12}, \cdots, x_{1j}, \cdots, x_{1n}, x_{21}, x_{22}, \cdots, x_{ij}, \cdots, x_{Nn})^T  \nonumber \\
	           &=& ({\bf x}_1^T, {\bf x}_2^T, \cdots, {\bf x}_i^T, \cdots, {\bf x}_N^T)^T  \nonumber
\end{eqnarray}
\begin{eqnarray}
	\label{weight_vector}
	{\bf w} &=& (w_{11}, w_{12}, \cdots, w_{1j}, \cdots, w_{1n}, w_{21}, w_{22}, \cdots, w_{ij}, \cdots, w_{Nn})^T  \nonumber \\
	           &=& ({\bf w}_1^T, {\bf w}_2^T, \cdots, {\bf w}_i^T, \cdots, {\bf w}_N^T)^T  \nonumber
\end{eqnarray}
パーセプトロンの目標は, 入力がクラス1に属しているとき出力が1, クラス0に属しているとき出力が0となるような重み${\bf w} $の集合を見つけることであり, その重みは, 式\ref{perceptron_learning_low}で表されるパーセプトロン学習則に従い, 自動的に変更される.
\begin{eqnarray}
	\label{perceptron_learning_low}
	{\bf w}_j^{new} = {\bf w}_j^{old} + \Delta {\bf w}_j 
\end{eqnarray}
ここで$\Delta {\bf w}_j $は, 正解クラスラベル$y$, 推定されたクラスラベル$y'$, 学習率$\ eta(0 \leq \eta \leq 1)$を用いて,以下のようにあらわすことができる.
\begin{eqnarray}
	\label{perceptron_learning_low}
	\Delta{\bf w}_j = \eta(y - y'){\bf x}_j^i
\end{eqnarray}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
パーセプトロンが開発されて数年後, B. Widrowと彼の大学院生たちにより, ADALINE(Ada)と呼ばれるパーセプトロンの改良版が発明された. 
ADALINEは, 図\ref{fig:adaline}に示す通り, 出力の二値が1もしくは-1であること, 学習則に平均二乗和誤差を用いたことがパーセプトロンと異なる,
\begin{figure}[ht]
	\begin{center}
		\includegraphics[clip, width=6cm]{./figure/adaline.eps}
		\caption{ADALINEの構造}
		\label{fig:adaline}
	\end{center}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
非線形問題を扱うために, 1986年D. Rumelhartによって考案されたニューラルネットワークが多層パーセプトロン(図\ref{NeuralNet})である. 
多層パーセプトロンは, 入力層, 出力層, そして中間層(隠れ層)で構成されており, 入出力層と隠れ層1層の場合が最も単純な構造である.
そして, 各層は内部にいくつかのノードと呼ばれる処理要素を持ち, 層ごとにその機能が異なる.

入力層のノードは, 自信と接続されているすべてのノードに対して, 同じ入力データを伝達する役割を持つ.
隠れ層のノードは, パーセプトロンと同様の構造を持ち, 活性化関数にシグモイド関数やReLU関数(\ref{sec:activation_function}節を参照)を用いる.
出力層のノードも基本的な構造は, パーセプトロンと変わらないが, 解きたい問題に応じて使用する活性化関数が変化する.
例えば, 回帰問題では恒等関数, 2クラス分類ではシグモイド関数, 多クラス分類ではソフトマックス関数を用いるのが一般的である.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{活性化関数}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\label{sec:activation_function}
活性化関数(または伝達関数)とは, 各ノードにおいて、入力信号の総和を出力信号に変換する関数のことであり, 入力をどのように活性化するかを決定する役割がある.
この節では, 本稿に登場するいくつかの活性化関数について説明する.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{ヘヴィサイドの階段関数}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ヘヴィサイドの階段関数は, $x<0$または$x>0$の範囲で連続であり, それぞれ$1, 0$を返す. また, $x=0$で値$c$をとるものとして階段関数
\begin{equation}
	\label{step_function}
	y_c(x) = \begin{cases}
			1  & \hspace{4mm} (x > 0 )\\
		...
		\end{cases}
\end{equation}
を実数全体の集合$\mathbb{R}$上の関数$y_c : \mathbb{R} \rightarrow \mathbb{R}$と考えるならば, $c$をどのように定めても原点$x=0$で不連続である. 
$c$の値は必要に応じて選ぶことができるが, $c = 0, c=1/2$などがしばしば用いられる.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{シグモイド関数}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
シグモイド関数は, 以下の式で表される単調増加する非線形関数である.
\begin{equation}
	\label{sigmoid_function}
	S(x) = \frac{1}{1+e^{-x}} \nonumber
\end{equation}
前述の階段関数が$0$か$1$のどちらかのみ出力するのと異なり, シグモイド関数は滑らかであるため, 実数 ---$0.731 \cdots$や$0.880, \cdots$など---を出力する. 
しかし, 入力信号が重要な情報であれば大きな値を出力し, 入力信号が重要でなければ小さな値を出力するという点, どんなに入力信号の値が大きくても, 出力信号を0から1の間に押し込める点,
ともに非線形関数である点においてこの2つの関数は同じ特徴を持つ. 
また, シグモイド関数は, その微分が
\begin{eqnarray}
	\label{sigmoid_function_differential}
	S'(x) &=& \frac{e^{-x}}{(1+e^{-x})^2} \nonumber \\ 
	       &=& S(x)(1-S(x)) \nonumber
\end{eqnarray}
と表されるため, ニューラルネットワーク内で微分を計算する場合も簡単に実装できる.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{ReLU関数}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ReLU(Rectified Linear Unit)もしくはランプ関数は, 
\begin{equation}
	\label{sigmoid_function}
	R(x) = \begin{cases}
			x  & \hspace{2mm} (x \geq 0 )\\
			0  & \hspace{2mm} (x < 0) \nonumber
		\end{cases}
\end{equation}
で表される, 原点$x=0$において不連続な非線形関数である. 
シグモイド関数や階段関数は, プラスマイナス∞の極限で$0, 1$をもつため, 入力信号の値が大きくなると微分による勾配が緩やかになるため, ニューラルネットワークの学習スピードが遅くなったり,
学習が進まなくなったりする. これに対し, ReLUは$x>0$の部分で微分値が常に$1$となるため, 勾配消失が起こらない.
しかし, $x<0$の場合は, 出力が0に統一されるため, 入力信号の特徴が失われ, ニューラルネットワークの学習が進みずらくなるといった問題がある.

この問題や$x=0$で微分不可能といった問題を解決するため, ReLU関数に似たいくつかの関数が存在する.
ここでは, それらの一種であるLeaky ReLUについて記載する.
\begin{equation}
	\label{sigmoid_function}
	R'(x) = \begin{cases}
			x  & \hspace{2mm} (x > 0 )\\
			0.01x  & \hspace{2mm} (x \leq 0) \nonumber
		\end{cases}
\end{equation}
...
\section{ネットワークの最適化}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
学習能力を持ったを持った多くの（すべてではない）ニューラルネットワークにおいて, 学習は, 処理要素の重みの変更を通して行われる.
そして, 重み訓練を行うニューラルネットワークにとって, その目標は, 希望のネットワーク性能を与える最適な重み${\bf w}$を見つけることである（このような問題を最適化問題という）.


学習能力を持ったを持った多くの（すべてではない）ニューラルネットワークにおいて, 学習は, 処理要素の要素の重みの変更を通して行われる.
そして, 重み訓練を行うニューラルネットワークにとって, その目標は, 希望のネットワーク性能を与える位置に, 重み${\bf w}$を導く学習則を開発することである. 
教師あり学習における希望のネットワーク性能とは, 一般的にネットワークの計算値と実際の結果により構成されるコスト関数

この学習則の多くは, はっきりとした目標のもとに定式化される. 一般に多く見かけるタイプの目標は, ニューラルネットワークの平均2乗和誤差や, 〇〇のような全体的コストまたは評価関数を最小または最大にすることである（このような学習則は性能学習(performance learning)と呼ばれるカテゴリに属する）. ある時間区間の入力信号の平均(または平均以外の性質)を学習するような, 特定のコスト関数の最適化を目標としない学習則は, 典型的な行動学または数学の用語で表すことができる目標を持つ(これはフィルタ学習(filter learning)カテゴリに属する学習則の目標である). 
ここでは, 実験に関係のある, 性能学習について説明する.
